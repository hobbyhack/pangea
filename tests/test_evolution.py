...
class TestEvolution:
    ...
    def test_mutate_traits_returns_five_values(self):
        """mutate_traits should return a 5-tuple including lifespan."""
        dna = DNA.random()
        result = mutate_traits(dna)
        assert len(result) == 5
    ...
    def test_create_next_generation_has_lifespan(self):
        """All DNA in next generation should have a lifespan trait."""
        top_dna = [DNA.random() for _ in range(5)]
        next_gen = create_next_generation(top_dna, population_size=20)
        for dna in next_gen:
            assert hasattr(dna, "lifespan")
