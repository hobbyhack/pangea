...
class TestDNA:
    ...
    def test_effective_lifespan(self):
        """Effective lifespan should include base + scaled lifespan points."""
        dna = self._make_dna(lifespan=20)
        expected = LIFESPAN_BASE + 20 * LIFESPAN_SCALE
        assert dna.effective_lifespan == pytest.approx(expected)

    def test_effective_lifespan_scales_with_points(self):
        """Higher lifespan points should yield longer effective lifespan."""
        low = self._make_dna(lifespan=5)
        high = self._make_dna(lifespan=50)
        assert high.effective_lifespan > low.effective_lifespan
    ...
    def test_to_dict_from_dict_round_trip(self):
        ...
        original = self._make_dna(25, 20, 20, 15, 20)
    ...
    def test_to_dict_contains_lifespan(self):
        """to_dict output should include the lifespan field."""
        dna = self._make_dna(lifespan=30)
        d = dna.to_dict()
        assert "lifespan" in d
        assert d["lifespan"] == 30

    def test_from_dict_backward_compat(self):
        """from_dict should handle old DNA dicts missing the lifespan key."""
        dna = self._make_dna(25, 25, 25, 25, 0)  # lifespan=0 just for weights
        d = dna.to_dict()
        # Remove lifespan to simulate old format
        del d["lifespan"]
        restored = DNA.from_dict(d)
        # Should fall back to DEFAULT_LIFESPAN
    ...
    def test_random_has_lifespan_trait(self):
        """Random DNA should include a lifespan trait >= 1."""
        for _ in range(20):
            dna = DNA.random()
            assert dna.lifespan >= 1
