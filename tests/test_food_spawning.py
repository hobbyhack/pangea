...
class TestFoodClusters:
    ...
    def test_cluster_size_one_is_single(self):
        ...
        world.spawn_food(1.0)
        # With rate=100 and seasonal multiplier near 1.0 at time=0,
        # we expect roughly 100 * seasonal_mult food items (each cluster = 1)
        assert len(world.food) >= 1
        # Each "cluster" is exactly 1 item, so count should equal number of triggers
        # No remainder constraint needed since cluster_size=1
...
class TestSeasonalFood:
    ...
    def test_season_affects_food_count(self):
        ...
        # At peak season (t = season_length/4), more food should spawn
        # than at trough (t = 3*season_length/4)
